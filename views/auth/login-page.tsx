...
import {
  ...
  CardContent,
  CardFooter,
} from "@statica/ui/components/ui/card";
...
import {
  ...
  InputOTPGroup,
  InputOTPSlot,
} from "@statica/ui/components/ui/input-otp";
...
import type { User } from "@statica/core/types";
    ...
    api
      ...
      .catch(() => {
        ...
        api
          ...
          .catch(() => {
            api.setToken(null);
            localStorage.removeItem("statica_token");
          });
      });
  }, [cliCallback]);
    ...
    async (value: string) => {
      ...
      try {
        if (cliCallback) {
          ...
          localStorage.setItem("statica_token", token);
          api.setToken(token);
          onTokenObtained?.();
  ...
  const handleCliAuthorize = async () => {
    ...
    try {
      ...
      if (authSourceRef.current === "localStorage") {
        ...
        const stored = localStorage.getItem("statica_token");
      ...
      <div className="flex min-h-svh items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Allow the CLI to access Statica as{" "}
  ...
  return (
    <div className="flex min-h-svh items-center justify-center">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center">
          {logo && <div className="mx-auto mb-4">{logo}</div>}
          <CardTitle className="text-2xl">Sign in to Statica</CardTitle>
          <CardDescription>
